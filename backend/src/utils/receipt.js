...
async function genererRecuPDF(livraison, urlVerification, appName = 'Gaz') {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (livraison.signatureDataUrl) {
        try {
          const base64 = livraison.signatureDataUrl.split(',').pop();
      ...
      const qrDataUrl = await QRCode.toDataURL(urlVerification, { margin: 1, width: 200 });
      ...
      doc
        ...
        .fillColor(GRIS)
        .text(
          `${appName} — Document généré automatiquement.`,
          50,
          doc.page.height - 50,
